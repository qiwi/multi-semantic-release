...
function parseCommit(commit) {
	...
	lines.forEach((l) => {
		if (l.startsWith("* ") || !answer.length) {
			if (answer.length) {
				answer[answer.length - 1].message = answer[answer.length - 1].message.trim();
			}
			answer.push({
				...commit,
				subject: l.startsWith("* ") ? l.slice(2) : l,
				message: l.startsWith("* ") ? l.slice(2) : l,
				squash: l.startsWith("* "),
			});
		} else {
			answer[answer.length - 1].message = answer[answer.length - 1].message + "\n" + l;
		}
	});
...
async function getCommitsFiltered(cwd, dir, lastHead = undefined, firstParentBranch) {
	...
	// Return the commits.
	// console.log('\n\ncommits QIWI;;', commits, '\n\n');
	// return commits;
	const unsquashedCommits = commits.reduce((commits, commit) => {
		const unsquashed = parseCommit(commit);
		if (unsquashed) {
