...
function getWorkspacesYarn(cwd, ignorePackages = null) {
	...
	// If packages to be ignored come from CLI, we need to remove the ones from manifest and add the ones from ignorePackages
	if (ignorePackages && Array.isArray(ignorePackages)) {
		packages = packages.filter((p) => !/^!/.test(p));
