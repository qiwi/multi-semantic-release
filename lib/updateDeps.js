...
const { getHighestLowest, getLatestVersion } = require("./utils");
...
const getNextPreVersion = (pkg, tags) => {
	...
	const lastVersion = pkg._lastRelease && pkg._lastRelease.version;
	let version = `1.0.0-${pkg._preRelease}.1`;

	if (pkg.name) tagFilters.push(pkg.name);
	if (!tags || !tags.length) tags = getTags(pkg._branch || "master", { cwd: "." }, tagFilters);

	if (lastVersion) {
		const { major, minor, patch, prerelease } = semver.parse(lastVersion);
		...
		if (semver.prerelease(lastVersion)) {
			...
			const bumpLatestVerTags = latestVerTags ? semver.inc(latestVerTags, "prerelease", pkg._preRelease) : null;
			const bumpLastVer = semver.inc(lastVersion, "prerelease", pkg._preRelease);

			version = bumpLatestVerTags ? getHighestLowest(bumpLastVer, bumpLatestVerTags) : bumpLastVer;
		} else {
			// Case when last version was a normal release and now it needs to be a prerelease
			version = `${semver.inc(`${major}.${minor}.${patch}`, pkg._nextType || "patch")}-${pkg._preRelease}.1`;
		}
	}
